...
async function startPaymentStatusPolling(invoiceNumber) {
    ...
    if (kDebugMode) console.log(`Starting payment status polling for ${invoiceNumber}`);
...
async function checkPaymentStatusAndRedirect(invoiceNumber) {
    try {
        ...
        if (response.documents.length > 0) {
            ...
            if (kDebugMode) console.log(`Payment status: ${status}`);

            // Handle status changes
            if (status === 'confirmed' || status === 'paid') {
                // Payment successful!
                clearInterval(pollingInterval);
                if (kDebugMode) console.log('Payment successful! Redirecting...');
                window.location.href = `payment-success.html?order_id=${order.$id}`;
            } else if (status === 'cancelled' || status === 'failed') {
                // Payment failed/cancelled
                clearInterval(pollingInterval);
                if (kDebugMode) console.log('Payment failed/cancelled. Redirecting...');
...
function stopPaymentPolling() {
    if (pollingInterval) {
        ...
        if (kDebugMode) console.log('Payment polling stopped');
    }
}
