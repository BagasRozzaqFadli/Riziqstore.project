...
function fuzzySearchLevenshtein(query, products, thresholdPercent = 0.40) {
    ...
    const results = products.map(product => {
        ...
        // IMPROVED threshold calculation for better typo tolerance
        // For short queries (like "nsi"), we need to be more forgiving
        const queryThreshold = Math.floor(normalizedQuery.length * thresholdPercent);
        const nameThreshold = Math.floor(normalizedName.length * thresholdPercent);

        // For very short queries (1-3 chars), add bonus tolerance
        let bonusTolerance = 0;
        if (normalizedQuery.length <= 3) {
            bonusTolerance = 1; // Allow 1 extra character difference
        } else if (normalizedQuery.length <= 5) {
            bonusTolerance = 0; // Standard tolerance
        }

        // Use the MAXIMUM threshold + bonus to be more forgiving
        const maxDistance = Math.max(queryThreshold, nameThreshold) + bonusTolerance;

        // Calculate similarity score (0-1)
        const similarity = calculateSimilarity(normalizedQuery, normalizedName);

        // Substring matching only for queries >= 2 characters
        // This allows "es" to match "Es Teh Manis" (desired)
        // Trade-off: "es" may also match "spesial" (acceptable for usability)
        const minSubstringLength = 2;
        const isSubstring = (normalizedQuery.length >= minSubstringLength) &&
            normalizedName.includes(normalizedQuery);

        // Determine if it's a match: either within threshold OR is substring
        const isMatch = (distance <= maxDistance) || isSubstring;
...
function printLevenshteinMatrix(source, target) {
    ...
    if (!debug.matrix) {
        if (kDebugMode) console.log(`One of the strings is empty. Distance: ${debug.distance}`);
        return;
    }
