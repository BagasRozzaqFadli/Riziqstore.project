...
/**
 * Debug mode flag is defined in config.js
 * This file uses the global kDebugMode constant
 */
...
function initializeSearch(products) {
    ...
    // Search button click - trigger search AND filter main list
    if (searchBtn) {
        searchBtn.addEventListener('click', () => {
            ...
            if (query.length >= MIN_SEARCH_LENGTH) {
                ...
                // Filter main product list
                if (results.length > 0 && typeof displayProducts === 'function') {
                    const matchedIds = results.map(r => r.product.$id);
                    const filteredProducts = allProducts.filter(p => matchedIds.includes(p.$id));
                    displayProducts(filteredProducts);
                    hideDropdown();

                    if (kDebugMode) console.log(`Filtered via icon click: ${filteredProducts.length} products`);
                } else {
                    performSearch(query); // Fallback: just show dropdown
                }
            }
        });
    }
...
function onSearchKeyDown(event) {
    ...
    // Enter key: Apply filter to main product list
    if (event.key === 'Enter') {
        ...
        if (query.length >= MIN_SEARCH_LENGTH) {
            ...
            if (results.length > 0) {
                ...
                // Call global displayProducts if available
                if (typeof displayProducts === 'function') {
                    // Filter products to show only matches
                    const filteredProducts = allProducts.filter(p => matchedIds.includes(p.$id));
                    displayProducts(filteredProducts);
                    ...
                    if (kDebugMode) console.log(`Filtered main list to ${filteredProducts.length} products`);
                } else {
...
function clearSearch() {
    ...
    // Reset main product list to show all products
    if (typeof displayProducts === 'function') {
        displayProducts(allProducts);
        if (kDebugMode) console.log('Reset to show all products');
    }
}
...
function performSearch(query) {
    ...
    if (cached) {
        ...
        if (kDebugMode) {
            ...
            if (kDebugMode) console.log(`Search time: ${(endTime - startTime).toFixed(2)}ms (from cache)`);
        }
        return;
    }
